...
1. **Clone the Repository**:
   ```bash
   git clone https://github.com/neel0210/celwrite.git
   cd celwrite
   ```

2. **Run a Static Server**:
   Since this is a modern ES module application, you need a local server to run it.
   ```bash
   npx serve .
   ```
...
3. **Configure API Key (Local)**:
   The app expects `process.env.API_KEY`. For local development without a bundler, you can temporarily mock this in `services/geminiService.ts` or ensure your environment injects it.
...
jobs:
  build-and-deploy:
    ...
    steps:
      ...
      # Add build steps here if using a bundler (Vite/Webpack)
      # Ensure process.env.API_KEY is replaced during build or handled by your runner
...
CelWrite evaluates your responses based on:
1. **Task Achievement**: Content relevance and completeness.
2. **Coherence & Cohesion**: Logical flow and transition usage.
3. **Vocabulary**: Precision and lexical range.
4. **Grammar & Accuracy**: Sentence variety and error density.
5. **Tone & Formality**: Audience-appropriate language.
